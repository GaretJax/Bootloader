...
``download [-b app-name [-d]]``
  ...
  -b app-name            Führt den Befehl ``burn app-name`` aus.
...
``burn app-name [-d]``
  ...
  Speichert das sich im Arbeitsspeicher befindende SRecordFile in den Flashspeicher unter den namen ``app-name``.
...
``set [--default=app-id] [--timeout=seconds] [...]``
  ...
  --errcount=num         Anzahl Neustarts im Falle eines fehlerhafen Starts einer Applikation.  Sobald errorcount erreicht ist, wird das Timeout deaktiviert und den Bootloader wird in Terminal-Mode umschalten.
